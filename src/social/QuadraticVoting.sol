...
contract QuadraticVoting is Owned {
    ...
    event PollCompleted(uint256 indexed _pollId, uint256 _result, uint256 _amountDonated);
    ...
    event VoterRemoved(uint256 indexed _pollId, address indexed _voter);
    ...
    struct Poll {
        uint256 totalWeiPaid; // Total amount paid into the poll (in Wei)
        uint256 totalWeiRefunded; // Amount of Wei paid out after revealed votes
        ...
        mapping(uint256 => Candidate) candidates; // Mapping of candidates in the election
    ...
    needs to be limited.
    */
    modifier onlyAdmin(uint256 _pollId) {
        require(polls[_pollId].admin == msg.sender, "msg.sender must be Poll Admin.");
        _;
    }
    ...
    modifier createdPhase(uint256 _pollId) {
        /* If it is past poll start time, but poll is still in Created 
        Phase, change to Commit Phase */
        if (polls[_pollId].status == PollStatus.created && block.timestamp >= polls[_pollId].startTime) {
            polls[_pollId].status = PollStatus.commitPhase;
        } else {
            require(polls[_pollId].status == PollStatus.created, "Poll must be in the Created phase.");
            _;
        }
    }

    modifier createdOrCommitPhase(uint256 _pollId) {
        /* If it is past poll start time, but poll is still in Created 
        Phase, change to Commit Phase */
        if (polls[_pollId].status == PollStatus.created && block.timestamp >= polls[_pollId].startTime) {
            polls[_pollId].status = PollStatus.commitPhase;
        ...
        if (polls[_pollId].status == PollStatus.commitPhase && block.timestamp >= polls[_pollId].closeTime) {
            polls[_pollId].status = PollStatus.revealPhase;
        } else {
            // Check that poll is in Created or Commit Phase
            require(
                polls[_pollId].status == PollStatus.created || polls[_pollId].status == PollStatus.commitPhase,
    ...
    modifier commitPhase(uint256 _pollId) {
        ...
        if (polls[_pollId].status == PollStatus.commitPhase && block.timestamp >= polls[_pollId].closeTime) {
            ...
            require(polls[_pollId].status == PollStatus.commitPhase, "Poll must be in Commit phase.");
            _;
        }
    }

    modifier revealPhase(uint256 _pollId) {
        /* If it is past poll closeTime, but poll is still in Commit
        Phase, change to Reveal Phase */
        if (polls[_pollId].status == PollStatus.commitPhase && block.timestamp >= polls[_pollId].closeTime) {
            polls[_pollId].status = PollStatus.revealPhase;
        ...
        if (
            polls[_pollId].status == PollStatus.revealPhase &&
            block.timestamp >= polls[_pollId].closeTime + (polls[_pollId].closeTime - polls[_pollId].startTime)
        ) {
            polls[_pollId].status = PollStatus.completed;
        } else {
            // Check that poll is in revealPhase
            require(polls[_pollId].status == PollStatus.revealPhase, "Poll must be in Reveal phase.");
            _;
        }
    }

    modifier completePhase(uint256 _pollId) {
        /*If it is past poll closeTime + (closeTime - startTime), but
        ...
        phase. */
        if (block.timestamp >= polls[_pollId].closeTime + (polls[_pollId].closeTime - polls[_pollId].startTime)) {
            polls[_pollId].status = PollStatus.completed;
        }
        require(polls[_pollId].status == PollStatus.completed, "Poll must be in Completed Phase.");
        _;
    }

    modifier onlyApprovedVoters(uint256 _pollId) {
        require(polls[_pollId].approvedVoters[msg.sender] == true, "msg.sender must be approved to vote in the poll.");
        _;
    }
    ...
    */
     ...
     */
    function addCandidate(uint256 _pollId, bytes32 _name)
        public
        onlyAdmin(_pollId)
     ...
     */
    function approveVoter(uint256 _pollId, address _voter)
        public
        stopInEmergency
        onlyAdmin(_pollId)
        createdOrCommitPhase(_pollId)
     ...
     */
    function transferAdmin(uint256 _pollId, address _newAdmin)
        public
        stopInEmergency
        onlyAdmin(_pollId)
        returns (address)
    ...
    */
    function removeApprovedVoter(uint256 _pollId, address _voter)
        public
        stopInEmergency
        onlyAdmin(_pollId)
        createdOrCommitPhase(_pollId)
    ...
    */
    function completePoll(uint256 _pollId) public completePhase(_pollId) returns (uint256) {
        uint256 topChoice;
        uint256 donationAmount = polls[_pollId].totalWeiPaid - polls[_pollId].totalWeiRefunded;
        for (uint256 i = 1; i <= polls[_pollId].candidateCount; i++) {
            if (
                polls[_pollId].candidates[i].voteCount != 0 &&
                polls[_pollId].candidates[i].voteCount == polls[_pollId].candidates[topChoice].voteCount
            ) {
                emit PollCompleted(_pollId, 0, donationAmount);
                payable(polls[_pollId].charity).transfer((donationAmount));
                return 0;
            }
            if (polls[_pollId].candidates[i].voteCount > polls[_pollId].candidates[topChoice].voteCount) {
                topChoice = i;
            }
        }
    ...
    function commitVote(
        ...
        bytes32 commitment,
        uint32 _numVotes
    ) public payable stopInEmergency onlyApprovedVoters(_pollId) commitPhase(_pollId) returns (bool) {
        ...
        votes at that time) */
        require(votes[msg.sender][_pollId].numVotes == 0, "Can only commit votes once.");

        // voteCost = numVotes**2 * firstVoteCost
        uint256 voteCost = (_numVotes * (_numVotes)) * (polls[_pollId].firstVoteCost);

        // Check that voter paid enough for the desired number of votes
        require(msg.value == voteCost, "msg.value must equal the cost of the votes committed.");
        ...
        polls[_pollId].totalVotesCommitted = polls[_pollId].totalVotesCommitted + (_numVotes);
        polls[_pollId].totalWeiPaid = polls[_pollId].totalWeiPaid + (msg.value);
        return true;
    }
    ...
    function revealVote(
        ...
        uint256 _vote,
        bytes32 _salt
    ) public stopInEmergency onlyApprovedVoters(_pollId) revealPhase(_pollId) returns (uint256) {
        ...
        bytes32 revealHash = keccak256(abi.encodePacked(_pollId, uint256(numVotes), _vote, _salt));
        require(revealHash == committedVote.commitment, "Hash does not match the committed hash.");
        ...
        candidate */
        require(_vote > 0 && _vote <= polls[_pollId].candidateCount, "Vote must be placed for a valid candidate.");
        uint256 _voteCount = uint256(polls[_pollId].candidates[_vote].voteCount);
        _voteCount = _voteCount + (numVotes);
        ...
        uint256 voteRevealerRefund = (polls[_pollId].totalWeiPaid) / (polls[_pollId].totalVotesCommitted);
        uint256 totalReturned = polls[_pollId].totalWeiRefunded;
        totalReturned = totalReturned + (voteRevealerRefund * numVotes);
        polls[_pollId].totalWeiRefunded = totalReturned;

        // Refund the voter for revealing their vote
        payable(msg.sender).transfer(voteRevealerRefund * numVotes);
    ...
    function getPollState(uint256 _pollId) public view returns (PollStatus) {
    ...
    function getCandidateInfo(uint256 _pollId, uint256 _candidateId)
        public
        view
        returns (uint32 voteCount, bytes32 name)
    {
        voteCount = polls[_pollId].candidates[_candidateId].voteCount;
        name = polls[_pollId].candidates[_candidateId].name;
    }

    function isUserVoter(uint256 _pollId, address _user) public view returns (bool) {
    ...
    function withdraw(
        ...
        uint256 _vote,
        bytes32 _salt
    ) public onlyApprovedVoters(_pollId) onlyInEmergency returns (uint256) {
        // Check that hash(pollId, vote, numVotes, salt) == commitment
        Vote memory committedVote = votes[msg.sender][_pollId];
        uint32 numVotes = committedVote.numVotes;
        bytes32 committedHash = committedVote.commitment;
        bytes32 revealHash = keccak256(abi.encodePacked(_pollId, uint256(numVotes), _vote, _salt));
        require(revealHash == committedHash, "Hash does not match the committed hash.");
    ...
    function hash(
        ...
        require(_vote <= polls[_pollId].candidateCount, "Vote must be placed for a valid candidate.");
