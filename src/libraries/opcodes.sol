...
// 20 => Cryptography
{
	uint8 constant KECCAK256 = 0x20;
}
// 30 - 48 => Blockchain info
{
...
// 50 - 5b => Storage
{
	...
	uint8 constant JUMPI = 0x57; ///< conditionally alter the program counter
	uint8 constant GETPC = 0x58; ///< get the program counter
	uint8 constant MSIZE = 0x59;
	uint8 constant GAS = 0x5a; ///< get the amount of available gas
	uint8 constant JUMPDEST = 0x5b; ///< set a potential jump destination
}
// 60-9f => Push, duplicate and swap
{
...
// b0 - ba => Tentatives
{
	uint8 constant JUMPTO = 0xb0; ///< alter the program counter to a jumpdest -- not part of Instructions.cpp
	uint8 constant JUMPIF = 0xb1; ///< conditionally alter the program counter -- not part of Instructions.cpp
	uint8 constant JUMPSUB = 0xb2;
	uint8 constant JUMPSUBV = 0xb4;
	uint8 constant BEGINSUB = 0xb5;
	uint8 constant BEGINDATA = 0xb6;
	uint8 constant RETURNSUB = 0xb8;
	uint8 constant PUTLOCAL = 0xb9;
	uint8 constant GETLOCAL = 0xba;
}
//e1 - e3 => Storage
{
	uint8 constant SLOADBYTES = 0xe1;
	uint8 constant SSTOREBYTES = 0xe2;
	uint8 constant SSIZE = 0xe3;
}
// f0 - ff => EVM methods
{
